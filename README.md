...
Next.js는 빌드 시 앱(프로젝트)을 전체적으로(**클라이언트 컴포넌트 포함**) 분석해서 가능한 한 정적페이지로 만들 수 있는 것들을 정적 HTML로 미리 만들어 둔다.

그래서 사용자가 홈페이지에 접속하면 Next.js가 미리 만들어둔 html을 먼저 클라이언트로 보낸다.

여기서 보내진 html은 말 그대로 정적 html이라 자바스크립트를 이용한 것들은 할 수 없다.

그 후 미리 만들어진 html을 동작, 컴포넌트화 등 완전하게 동작 가능하게 하기 위해서 React 라이브러리 코드 등 필요한 자바스크립트 코드를 보내서 하이드레이션을 한다.

Next.js 13 버전부터는 **클라이언트/서버 컴포넌트를 이용**하여 **컴포넌트 단위로 쿨라이언트/서버 렌더링이 가능**하게 됐다.

-서버 컴포넌트

서버 컴포넌트는 처음 페이지 로딩을 빠르게 해주고 클라이언트 사이드의 자바스크립트 번들 사이즈를 줄여준다.

기본 클라이언트 측 런타임은 캐싱이 가능하고 크기가 예측 가능하며 애플리케이션이 커져도 증가하지 않는다.

추가적인 자바스크립트는 클라이언트 컴포넌트를 통해 애플리케이션에서 클라이언트 측 상호 작용이 필요할 때만 추가된다.

PHP와 비슷하게 실행 결과만을 뿌려주는 것이기 떄문에 DB연결하는 코드, API키가 포함되는 코드 같은 것을 서버 컴포넌트에 직접 작성할 수 있다.

-클라이언트 컴포넌트

클라이언트 컴포넌트는 클라이언트 사이드의 상호작용을 가능하게 해 준다.

Next.js에서는 서버에서 prerender되고 클라이언트에서 hydrate된다.

위 내용 아래 서버/클라이언트 컴포넌트 항목에 융합해서 작성할 것.
...
5. 서버 컴포넌트는 처음 페이지 로딩을 빠르게 해주고 클라이언트 사이드의 자바스크립트 번들 사이즈를 줄여준다.

6. 기본 클라이언트 측 런타임은 캐싱이 가능하고 크기가 예측 가능하며 애플리케이션이 커져도 증가하지 않는다.

7. 추가적인 자바스크립트는 클라이언트 컴포넌트를 통해 애플리케이션에서 클라이언트 측 상호 작용이 될 때만 추가된다.
...
1. 클라이언트 컴포넌트라고 해서 **무조건 CSR(클라이언트 사이드 렌더링)이 되는 것이 아니다.**

2. 사용자와의 상호작용 이벤트(onCLick 등)를 처리한다던지 상태, 메모리 등 무언가 반드시 브라우저에서 실행되어야 하는 그 코드는 무조건 클라이언트 컴포넌트로 만들어야 한다.
