...
```
1. 사용자가 홈페이지에 접속하면 Next.js가 미리 만들어둔 html을 먼저 클라이언트로 보낸다.

2. 여기서 보내진 html은 말 그대로 정적 html이라 자바스크립트를 이용한 것들은 할 수 없다.

3. 그 후 미리 만들어진 html을 동작, 컴포넌트화 등 완전하게 동작 가능하게 하기 위해서 React 라이브러리 코드 등 필요한 자바스크립트 코드를 보내서 하이드레이션을 한다.
```
